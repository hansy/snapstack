...
export const SideZone: React.FC<SideZoneProps> = ({
    ...
    return (
        ...
        >
            <Zone zone={zone} className={cn(ZONE_SIDEWAYS_CLASSES, "bg-zinc-800/30 rounded-lg border-2 border-dashed border-zinc-700 flex items-center justify-center relative", onContextMenu && "cursor-context-menu")}>            
                {card ? (
                    <div className="w-full h-full relative overflow-hidden rounded-lg">
                        <Card
                            card={card}
                            faceDown={faceDown}
                            className={cn("absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rotate-90 scale-90 pointer-events-none origin-center", cardClassName)}
                        />
                    </div>
                ) : (
