...
export const formatMissingCards = (missing: ParsedCard[]): string => {
  return missing
    .map(
      (card) =>
        `${card.quantity}x ${card.name}${card.set ? ` (${card.set.toUpperCase()} ${card.collectorNumber})` : ""}`
    )
    .join(", ");
};
...
export const validateImportResult = (
  ...
  const expectedCount = parsedDeck.reduce(
...
const identifierMatchesParsedCard = (
  card: ParsedCard,
...
const mergeMissingCard = (
  missingMap: Map<string, ParsedCard>,
  card: ParsedCard
) => {
...
export const fetchScryfallCards = async (
  ...
  const cardsToCache: ScryfallCard[] = []; // Collect full cards for caching
  ...
  const markChunkAsMissing = (chunk: ScryfallIdentifier[]) => {
    parsedCards.forEach((card) => {
      if (
        chunk.some((identifier) =>
          identifierMatchesParsedCard(card, identifier)
        )
      ) {
  ...
  for (const chunk of chunks) {
    try {
      ...
      data.data.forEach((scryfallCard: ScryfallCard) => {
        // Cache full card data locally
        cardsToCache.push(scryfallCard);

        const originalRequest = parsedCards.find(
          (pc) =>
            (pc.set === scryfallCard.set &&
              pc.collectorNumber === scryfallCard.collector_number) ||
        ...
        if (originalRequest) {
          for (let i = 0; i < originalRequest.quantity; i++) {
            ...
            // Get power/toughness from card or front face
      ...
      warnings.push(
        `Error fetching from Scryfall: ${error instanceof Error ? error.message : String(error)}`
      );
      markChunkAsMissing(chunk);
    }
  }
  ...
  for (const missingCard of Array.from(missingMap.values())) {
    ...
    if (resolved) {
      ...
      for (let i = 0; i < missingCard.quantity; i++) {
        ...
        const power = resolved.power ?? frontFace?.power;
        const toughness = resolved.toughness ?? frontFace?.toughness;
        ...
        fetchedCards.push({
          ...
          // Store only lite version for sync efficiency
          scryfall: toScryfallCardLite(resolved),
          tapped: false,
          faceDown: false,
          currentFaceIndex: 0,
          rotation: 0,
  ...
  // Cache all fetched cards in IndexedDB (async, don't await)
  ...
  const expectedCardCount = parsedCards.reduce(
    (sum, card) => sum + card.quantity,
    0
  );
  if (
    fetchedCards.length +
      missing.reduce((sum, card) => sum + card.quantity, 0) <
    expectedCardCount
  ) {
    warnings.push(
      "Some requested cards could not be resolved and were skipped."
    );
  }
